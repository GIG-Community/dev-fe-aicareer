...
const Interview = ({ onEnd }) => {
  ...
  const generateNewQuestion = async () => {
    ...
    try {
      const question = await geminiService.generateInterviewQuestion(jobRole, language, difficulty);
      setCurrentQuestion(question);
      setQuestionCount(prev => prev + 1);
      
      // Auto speak the question with longer delay for Arabic
      const speakDelay = language === 'ar' ? 2000 : 1000;
      
      setTimeout(async () => {
        try {
          console.log(`Speaking question in ${language}:`, question);
          await audioService.speak(question, language);
          console.log(`Finished speaking question in ${language}`);
        } catch (err) {
          console.error('Speech synthesis failed:', err);
          // Show user-friendly error for Arabic
          if (language === 'ar') {
            setError('Arabic speech synthesis is not available. Please read the question above.');
          }
        }
        setIsSpeaking(false);
      }, speakDelay);
  ...
  return (
    <div className="fixed inset-0 h-screen bg-gradient-to-br from-gray-50 to-blue-50 flex flex-col overflow-hidden z-50">
      {/* Header */}
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex justify-between items-center flex-shrink-0">
        <div className="flex items-center space-x-4">
          <h1 className="text-gray-800 font-medium">AI Interview Session</h1>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              {isSpeaking && (
                <div className="flex items-center text-green-600">
                  ...
                  <span className="text-sm">Speaking in {languages.find(l => l.code === language)?.nativeName}...</span>
                </div>
              )}
            </div>
            <div className="bg-gray-50 border border-gray-100 rounded-lg p-4">
              <p className="text-gray-800 text-lg leading-relaxed" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                {currentQuestion || 'Preparing your question...'}
              </p>
              {/* Audio indicator for Arabic */}
              {language === 'ar' && currentQuestion && (
                <div className="mt-2 text-sm text-gray-500" dir="rtl">
                  {isSpeaking ? '🔊 يتم التحدث بالسؤال...' : '📖 اقرأ السؤال أعلاه'}
                </div>
              )}
            </div>
          </div>
